...
public class MsgTimeoutTimer extends Timer {
    ...
    private BaseTcpMessage msg;// 发送的消息
    private int currentResendCount;// 当前重发次数
    private MsgTimeoutTask task;// 消息发送超时任务

    public MsgTimeoutTimer(IMSClientInterface imsClient, BaseTcpMessage msg) {
        ...
        public void run() {
            if (imsClient.isClosed()) {
                if (imsClient.getMsgTimeoutTimerManager() != null) {
                    imsClient.getMsgTimeoutTimerManager().remove(String.valueOf(msg.getHead().getMsgId()));
                }
...
//                    headBuilder.setStatusReport(TCPConfig.DEFAULT_REPORT_SERVER_SEND_MSG_FAILURE);
