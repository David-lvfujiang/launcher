 ...
 * Date:   2019/9/3 10:39
 */
@Route(path = RouterPath.SETTINGS.SettingsInitDeviceStatusActivity)
public class SettingsInitDeviceStatusManager {
    ...
    public boolean getStatus(Context context) {
        boolean status = AppUtils.isBindedDevice(context);

        return status;
    }

    public void setStatus(boolean deviceStatus) {
        this.deviceStatus = deviceStatus;
    }

    public void judgeviceStatus(Context context) {
        ...
        if (AppUtils.isRegisterDevice(context) ){
            ...
            if (AppUtils.isBindedDevice(context)) {
                ...
                String userId = (String) SPUtils.get(SettingsApplication.getInstance(), Constant.Settings.DEVICE_ID, "");
...
//        star(new Intent(context, SettingsBindDeviceActivity.class));
    }
...
//        }
        ...
        if (response.getData().getBindStatus()) {
            ...
            Intent intent = new Intent(context, SettingsBindDeviceActivity.class);
            context.startActivity(intent);
//            SPUtils.put(mContext, Constant.Settings.DEVICE_STATUS, false);
//            startActivity(new Intent(MainActivity.this, DeviceBind.class));
//            finish();
        }
    }
}
