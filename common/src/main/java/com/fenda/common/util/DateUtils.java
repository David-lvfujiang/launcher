...
// return d2.getMonth()+”月”+d2.getDay()+”日”;//formatTweetDetailTime(date, false);
    ...
    public static String getPhotoUploadDate(long timesamp) {
        String result = "";
        Calendar todayCalendar = Calendar.getInstance();
        Calendar otherCalendar = Calendar.getInstance();
        otherCalendar.setTimeInMillis(timesamp);
        String yearTimeFormat = "yyyy年MM/dd";
        boolean yearTemp = todayCalendar.get(Calendar.YEAR) == otherCalendar.get(Calendar.YEAR);
        if (yearTemp) {
            int temp = todayCalendar.get(Calendar.DATE) - otherCalendar.get(Calendar.DATE);
            if (temp == 0) {
     ...
     * @return
     */
    public boolean isSameDay(String str1, String str2) {
        ...
        DateFormat df1 = new SimpleDateFormat("EEE MMM dd HH:mm:ss Z yyyy", Locale.UK);
        DateFormat df2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date1 = null;
        Date date2 = null;
        try {
            date1 = df.parse(str1);
            date1 = df1.parse(date1.toString());
            date1 = df2.parse(date1.toString());
            date2 = df.parse(str2);
            date2 = df1.parse(date2.toString());
            date2 = df2.parse(date2.toString());
            calendar1.setTime(date1);
            calendar2.setTime(date2);

            int y1 = calendar1.get(Calendar.YEAR);//获取年份

            int d1 = calendar1.get(Calendar.DAY_OF_YEAR);//获取年中第几天
    ...
    public static String dealDateFormat(String oldDateStr) {
        ...
        try {
            ...
            return getPhotoUploadDate(date.getTime());
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return "";
